...
from background.libs import strings
...
class U2NET:
    ...
    def __init__(self, name="u2net"):
        ...
        from background.libs.u2net import U2NET as U2NET_DEEP
        from background.libs.u2net import U2NETP as U2NETP_DEEP
        ...
        try:
            if self.torch.cuda.is_available():
                net.load_state_dict(self.torch.load(os.path.join("background", "models", name, name + '.pth')))
                net.cuda()
            else:
                net.load_state_dict(self.torch.load(os.path.join("background", "models", name, name + '.pth'), map_location="cpu"))
